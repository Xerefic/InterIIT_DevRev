...
class GenerateQuestions:
    ...
    def generate_queries(candidates, text, pbar=False):
        ...
        for i in tqdm.trange(0, len(candidates), batch_size, disable=not pbar):
            ...
            outputs = self.model.generate(**features, max_length=max_length_query, do_sample=True,
                                     top_p=top_p, num_return_sequences=num_queries)
            queries.extend([tokenizer.decode(out, skip_special_tokens=True).strip('question: ') for idx, out in enumerate(outputs)])
        queries = list(set(queries))
        return queries
    
    def fit(self, pbar=False):
        for text in tqdm.tqdm(self.df.Paragraph.unique(), disable=not pbar):
            candidates = self.generate_candidates(text)
            self.question_bank[para] = self.generate_queries(candidates, text)
